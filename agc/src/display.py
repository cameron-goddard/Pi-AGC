...
class Display:
    ...
    def rect_init(self) -> None:
        ...
        self.label_list = [[comp_label], [prog_label], [verb_label], [noun_label]]
    ...
    def text_init(self) -> None:
        ...
        for i in range(len(self.row_list)):
            self.row_list[i].append(row_text[i])
            self.screen.blit(self.row_list[i][1], self.row_list[i][0])
        ...
        for i in range(len(self.box_list)):
            self.box_list[i].append(box_text[i])
            self.screen.blit(self.box_list[i][1], self.box_list[i][0])
        ...
        for i in range(len(self.label_list)):
            self.label_list[i].append(label_text[i])
            self.screen.blit(self.label_list[i][1], self.label_list[i][0])
    ...
    def blit_all(self):
        ...
        for item in self.box_list:
            pygame.draw.rect(self.screen, (0,0,0), item[0])
            self.screen.blit(item[1], item[0])
        pygame.draw.rect(self.screen, (0,0,0), self.label_list[0][0])
        self.screen.blit(self.label_list[0][1], self.label_list[0][0])
        for i in range(len(self.label_list)):
            if i != 0:
                pygame.draw.rect(self.screen, (0, 255, 1), self.label_list[i][0])
                self.screen.blit(self.label_list[i][1], self.label_list[i][0])
    
    def update_row(self, row: int, val: str) -> None:
        self.row_list[row][1] = self.font_dig.render(val, False, (0, 255, 1))
        self.screen.blit(self.row_list[row][1], self.row_list[row][0])
        self.blit_all()
        pygame.display.flip()
        

    def update_verb(self, val: str) -> None:
        self.box_list[1][1] = self.font_dig.render(val, False, (0, 255, 1))
        self.screen.blit(self.box_list[1][1], self.box_list[1][0])
        self.blit_all()
        pygame.display.flip()

    def update_noun(self, val: str) -> None:
        self.box_list[2][1] = self.font_dig.render(val, False, (0, 255, 1))
        self.screen.blit(self.box_list[2][1], self.box_list[2][0])
        self.blit_all()
        pygame.display.flip()

    def update_prog(self, val: str) -> None:
        self.box_list[0][1] = self.font_dig.render(val, False, (0, 255, 1))
        self.screen.blit(self.box_list[0][1], self.box_list[0][0])
